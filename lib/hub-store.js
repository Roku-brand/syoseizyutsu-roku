...
const SUPABASE_URL = 'https://otwnmfoubxdenqkrkxmx.supabase.co';
const SUPABASE_PUBLISHABLE_KEY = 'sb_publishable_cEI9jtk6n0S01-2dGaPn3A_MLT28InS';

export const assertSupabaseAdminRequest = async (request) => {
  ...
  if (!authResponse.ok) {
    throw new ApiError('ログイン情報を確認できません。再度ログインしてください。', 401);
  }
  const authUser = await authResponse.json();

  const profileResponse = await fetch(
    `${SUPABASE_URL}/rest/v1/profiles?id=eq.${encodeURIComponent(authUser.id)}&select=role&limit=1`,
    {
      headers: {
        apikey: SUPABASE_PUBLISHABLE_KEY,
        authorization: `Bearer ${accessToken}`,
  ...
  if (!profileResponse.ok) {
    throw new ApiError('管理者権限を確認できません。', 403);
  }
  const profiles = await profileResponse.json();
  if (profiles?.[0]?.role !== 'admin') {
    throw new ApiError('この操作は管理者本人のみ実行できます。', 403);
  }
  return authUser;
};
